...
        const jsonText = message.content.substring(offset).trim();

        if (!channelId || !jsonText)
          return message.reply(`Uso: \`!send #canal <JSON de Mensaje/Embed>\``);
...
          if (parsed.content) {
            sendOptions.content = parsed.content;
          }
...
        } catch (error) {
          message.reply(`Error en el JSON o al enviar el mensaje: ${error.message}`);
        }
        break;
      }
